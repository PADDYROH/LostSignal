...
public class RegistrationGetter {
	...
	public Player registerPlayer() {
		...
		int result = JOptionPane.showConfirmDialog(mainFrame, panel, "Register Player", JOptionPane.OK_CANCEL_OPTION,
		...
		if (result == JOptionPane.OK_OPTION) {
			...
			if (passwordField.getPassword().length < 1) {
				...
				temp.setFont(GUIModel.normalFont);
				JOptionPane.showMessageDialog(mainFrame, "Password must be entered!");
				return null;
			}
			// return player based on input
			return new BasePlayer(iDField.getText(), (String.valueOf(passwordField.getPassword())).hashCode(),
					nameField.getText(), 0);
